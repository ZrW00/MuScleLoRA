...
class ONIONDefender(Defender):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.LM = GPT2LM(parallel, device = device)
    ...
    def correct(
        ...
        for (poison_text, label, poison_label) in tqdm(poison_data, desc="correcting text"):
...
class GPT2LM():
    def __init__(self, parallel, device):
        ...
        path = "gpt2"
        self.device = device
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained(path)
        self.lm = transformers.GPT2LMHeadModel.from_pretrained(path).to(self.device)
        if parallel:
